...
from anomalib.data.utils.path import contains_non_printable_characters, validate_path
...
class TestValidatePath:
    ...
    @staticmethod
    def test_unicode_path_is_allowed() -> None:
        """Test Unicode paths are treated as printable."""
        assert not contains_non_printable_characters("D:/数据集/涂布/漏金属.bmp")

    @staticmethod
    def test_existing_unicode_file_within_base_dir() -> None:
        """Test ``validate_path`` accepts existing files under a Unicode path."""
        with TemporaryDirectory() as tmp_dir:
            base_dir = Path(tmp_dir) / "数据集"
            base_dir.mkdir()
            file_path = base_dir / "涂布图像.png"
