...
class PatchcoreModel(DynamicBufferMixin, nn.Module):
    ...
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor | InferenceBatch:
        ...
        if self.training:
            # Move to CPU immediately to avoid GPU OOM during long training.
            # subsample_embedding() will later stack them on CPU and move
            # the final (small) coreset result back to device.
            self.embedding_store.append(embedding.cpu())
            return embedding
    ...
    def subsample_embedding(self, sampling_ratio: float, embeddings: torch.Tensor = None) -> None:
        ...
        # Move all embeddings to CPU first to avoid GPU OOM during vstack,
        # then stack and (optionally) move back to the original device.
        device = self.memory_bank.device
        cpu_embeddings = [emb.cpu() for emb in self.embedding_store]
        self.embedding_store.clear()

        self.memory_bank = torch.vstack(cpu_embeddings)
        del cpu_embeddings  # free CPU memory early

        sampler = KCenterGreedy(embedding=self.memory_bank, sampling_ratio=sampling_ratio)
        self.memory_bank = sampler.sample_coreset().to(device)
